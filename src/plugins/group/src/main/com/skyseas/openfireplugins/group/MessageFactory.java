...
public class MessageFactory {
    ...
    public static Message newInstanceForGroupDestroy(Group group,String operatorName,JID operator, String reason) {
        return new MessageBuilder(ActionType.DESTROY)
                .setReason(reason)
                .setOperatorFromName(operatorName)
                .setOperatorFrom(operator.asBareJID())
                .setGroupName(group.getGroupInfo().getName())
    ...
    private static class MessageBuilder {
        ...
        public MessageBuilder setOperatorFromName(String fromName) {
            this.operatorFromName = fromName;
            return this;
        }
        ...
        public Message build() {
            ...
            if(operatorFrom != null) {
                ...
                extElement.addAttribute("fromName", operatorFromName.toString());
            }
            
            Element groupElement = extElement.addElement("group");
            if(groupName != null) {
            	groupElement.addAttribute("groupname", groupName);
            }
            if(groupElement.attributeCount() < 1) {
                extElement.remove(groupElement);
            }
