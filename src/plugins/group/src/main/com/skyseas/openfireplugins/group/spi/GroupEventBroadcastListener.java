...
public final class GroupEventBroadcastListener implements GroupEventListener {
    ...
    @Override
    public void groupDestroyed(final Group group, final ChatUser user, final String reason) {
        TaskEngine.getInstance().submit(new Runnable() {
            @Override
            public void run() {
                Message msg = MessageFactory.newInstanceForGroupDestroy(group,user.getUserName(),user.getJid(), reason);
                group.broadcast(msg);
            }
        });
